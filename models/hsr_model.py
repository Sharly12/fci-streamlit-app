...
def _load_raster(path):
    """Read single-band raster as float64 + profile, masking nodata to NaN."""
...
def _save_raster(arr, profile, out_path):
    """Save float raster with NaN nodata."""
...
def _reproject_raster(src_path, dst_path, target_crs, res):
    """Reproject src raster to target_crs at resolution `res` (m)."""
...
def run_hsr_analysis(
    ...
    depth_threshold_m: float | None = 2.0,
    target_resolution: float = 30.0,
    out_dir: str = "outputs/individual/hsr",
):
    """
    Run HSR ENGINE v4 logic and return (parcels_gdf, diagnostics).

    dem_path, cn_path, parcels_path : file paths (original CRS)
    rainfall_mm       : storm depth
    concavity_window  : morphological window (cells) for concavities
    depth_threshold_m : if None -> use median concavity depth (original behaviour)
                        if float -> fixed depth threshold in meters
    """

    os.makedirs(out_dir, exist_ok=True)

    # -------------------------------------------------------------
    # STEP 1 — Reproject DEM & CN to UTM at target_resolution
    # -------------------------------------------------------------
    ...
    cn_utm_path = os.path.join(out_dir, "cn_utm.tif")
    ...
    # -------------------------------------------------------------
    # STEP 3 — Concavity depth (storage_depth)
    # -------------------------------------------------------------
    ...
    storage_depth = dem_closed - dem_array  # metres
    ...
    if positive.size == 0:
        ...
        if depth_threshold_m is None:
            used_threshold = float(np.percentile(positive, 50))
        else:
            used_threshold = float(depth_threshold_m)

        concavity_mask = storage_depth >= used_threshold
    ...
    # -------------------------------------------------------------
    if n_deps == 0:
        storage_volumes = np.array([], dtype="float64")
    ...
    hsr_static_path = os.path.join(out_dir, "HSR_static.tif")
    ...
    runoff_path = os.path.join(out_dir, "runoff_weights.tif")
    ...
    # -------------------------------------------------------------
    # STEP 7 — Inflow & rainfall-adjusted storage (HSR_rain)
    # -------------------------------------------------------------
    if n_deps == 0:
        inflow_mm = np.array([], dtype="float64")
        inflow_volumes = np.array([], dtype="float64")
        HSR_rain_map = np.zeros_like(dem_array, dtype="float64")
        HSR_rain = np.array([], dtype="float64")
    else:
    ...
    hsr_rain_path = os.path.join(out_dir, "HSR_rain.tif")
    _save_raster(HSR_rain_map, dem_profile, hsr_rain_path)

    # -------------------------------------------------------------
    # STEP 8 — Parcel-level zonal stats
    # -------------------------------------------------------------
    ...
    # Ensure parcels in the same UTM CRS
    ...
    diagnostics = {
        ...
        "depth_threshold_used": float(used_threshold),
        "n_patches": int(n_deps),
        "total_storage_m3": float(np.nansum(storage_volumes)),
        "total_rainfilled_m3": float(np.nansum(HSR_rain)),
    }
