...
# ---- NumPy compatibility for pysheds ----
...
def run_uds_analysis(
    ...
    cn_vals = np.array([d.get("mean", np.nan) if isinstance(d, dict) else np.nan for d in zs], dtype="float64")
    parcels["CN"] = cn_vals

    # SCS–CN RUNOFF (unchanged)
    P = float(rainfall_mm)
    ...
    # ----------------------------
    # Export-only rasters for downloads (no analysis changes)
    # ----------------------------
    ...
    def _rasterize_metric(values, out_dtype="float32", nodata_val=-9999.0):
        shapes_val = ((geom, float(val)) for geom, val in zip(parcels.geometry, values))
        return rasterize(
            shapes_val,
            out_shape=(dem_height, dem_width),
            transform=dem_transform,
