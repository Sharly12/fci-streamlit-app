...
with st.sidebar:
    ...
    # Useful during debugging: PEC run is cached.
...
if run_btn:
    try:
        ...
        with c1:
            st.metric(
                "Parcels analysed", diagnostics.get("n_parcels", len(parcels_pec))
            )
            st.metric(
                "DEM resolution (m)",
        ...
        with c2:
            ...
            st.metric(
                "Radius (pixels)",
                diagnostics.get("neighbourhood_radius_pixels", 0),
            )
        with c3:
            st.metric(
        ...
        # Always show all 4 PEC classes (even if a class count is 0)
        risk_order = [
        ...
        # Extra diagnostics that explain why a class might disappear
        with st.expander("Why a class might be missing (debug)"):
            st.write(
                {
        ...
        table_df = (
            parcels_pec.sort_values(["pec_rank", "hand_score"])[existing_cols]
            .head(10)
            .copy()
        )
