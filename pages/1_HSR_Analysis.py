...
if run_btn:
    try:
        ...
        with st.spinner("Running HSR ENGINE v4 …"):
            # ✅ now receives outputs too (paths to the already-written tifs)
        ...
        # ------------------------------------------------------------------
        # ✅ Downloads: GeoTIFF + KML (NO analysis changes, just exporting outputs)
        # ------------------------------------------------------------------
        st.subheader("Downloads")

        # Read GeoTIFFs your model already produced
        with open(outputs["hsr_static_tif"], "rb") as f:
            static_bytes = f.read()
        with open(outputs["hsr_rain_tif"], "rb") as f:
            rain_bytes = f.read()

        # Optional ZIP (both rasters together)
        zip_buf = io.BytesIO()
